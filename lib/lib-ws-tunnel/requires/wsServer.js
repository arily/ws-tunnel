...
module.exports = class wsServer {
  constructor (wsConfig, { prefix = '/', serverName = 'Proxy', prefabRoute = [], proxifier }) {
  ...
  newConnection (src, req) {
    try {
      ...
      if (this.connections.has(connectionId)) {
        ...
        this.connections.set(connectionId, new WsTunnel({ src, protocol, port, addr, req, headers, server: this }))
      }
    } catch (error) {
      console.log(error)
      src.close()
